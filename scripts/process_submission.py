...
INTAKE_COMMENT_MARKER = "<!-- offmap-intake-parsed -->"

LEGACY_INTAKE_COMMENT_MARKERS = {
    "<!-- enchanted-map-intake -->",
    "<!-- enchanted-map-intake-parsed -->",
}
...
# GitHub Issue Forms render visible labels in the issue body.
# These values must exactly match submit-opportunity.yml.
FIELD_LABELS: dict[str, str] = {
    # Part I: Essential Opportunity Details
    "opportunity_name": "✨ Opportunity name",
    "main_category": "🗂️ Main opportunity category",
    ...
    "audience_groups": (
        "🌈 Intended, encouraged, or eligible groups"
    ),
    "audience_information": (
        "🫶 Exact audience or inclusion wording"
    ),
    ...
    "eligible_locations": (
        "🗺️ Specific eligible countries or regions"
    ),
    ...
    "application_requirements": (
        "🏹 Application requirements or selection process"
    ),
...
AUDIENCE_MAP = {
    ...
    "rural or remote-community students": (
        "rural-remote-students"
    ),
    "another specific community": (
        "other-specific-community"
    ),
...
def neutralize_mentions(value: str) -> str:
    """Prevent contributor-controlled text from creating mentions."""
...
def safe_comment_value(value: Any) -> str:
    ...
    if len(text) > MAX_COMMENT_VALUE_LENGTH:
        return (
            text[:MAX_COMMENT_VALUE_LENGTH].rstrip()
            + "…"
        )
...
def get_issue() -> dict[str, Any]:
    ...
    if issue.get("pull_request"):
        fail(
            "The supplied issue number refers "
            "to a pull request."
        )
...
def clean_single_value(
    ...
    # Preserve the raw issue itself exactly on GitHub. Refuse to
    # process extreme fields rather than silently modifying them.
...
def parse_multi_value(
    ...
    GitHub may render checkbox options as ``- [x] value`` and
    unchecked options as ``- [ ] value``. Unchecked values must
    not enter routing.
    """
    ...
    for raw_line in cleaned.splitlines():
        ...
        # Remove a Markdown bullet before checking checkbox syntax.
        line = re.sub(
            r"^[-*]\s+",
            "",
            line,
    ...
    # Some multi-select values may be rendered as one
...
def extract_raw_submission(
    ...
    for internal_name, visible_label in FIELD_LABELS.items():
        ...
        if internal_name in MULTI_VALUE_FIELDS:
            raw_submission[
                internal_name
            ] = parse_multi_value(raw_value)
        else:
            raw_submission[
                internal_name
            ] = clean_single_value(raw_value)
...
def looks_like_url(
    ...
    This rejects ordinary text, missing schemes, localhost
    addresses, private or loopback IP addresses, embedded
    credentials, and malformed ports.
    """
    ...
    try:
        ...
        # Accessing parsed.port validates malformed ports.
        _ = parsed.port
    except ValueError:
        return False
    ...
    try:
        ...
        # Reject localhost, private, reserved, and link-local IPs.
...
def parse_category_choice(
    value: Any,
) -> tuple[str | None, str]:
    """
    Convert a value such as ``Events — Conference`` into:

        ("events", "conference")

    The first result is the broad category implied by the
    specific dropdown. It is used only for consistency checking.

    ``Other or Not Sure`` has no implied broad category.
    """
    ...
    specific_category = CATEGORY_MAP.get(
        normalize_compare_text(subtype),
...
def basic_intake_warnings(
    ...
    if (
        official_website
        and not looks_like_url(
            official_website
        )
    ):
        warnings.append(
            "The official source does not appear to be "
    ...
    if (
        ...
        warnings.append(
            "The application link does not appear to be "
            "a complete HTTP or HTTPS URL."
        )
    ...
    if (
        ...
        and expected_main_category
        != selected_main_category
    ):
        warnings.append(
            "The selected main category does not match "
            "the specific opportunity type. "
            f"Main category: "
            f"'{raw_submission.get('main_category')}'. "
            f"Specific type implies: "
            f"'{expected_main_category}'."
        )

    # Deliberately do not inspect audience_information here.
    # Community categories come only from audience_groups.
    return warnings
...
def extract_keyword_hints(
    ...
    # audience_information is intentionally excluded.
    # It must not affect community classification or routing tags.
    source_fields = [
        "opportunity_name",
        "short_description",
        "specific_majors",
...
def load_moderator_configuration(
) -> dict[str, Any]:
    if not MODERATOR_FILE.exists():
        warn(
            f"Moderator registry not found at "
            f"{MODERATOR_FILE}. "
...
def get_open_assignment_count(
    ...
    if response.status_code != 200:
        warn(
            f"Could not check workload for "
            f"{username}: "
            f"{response.status_code}"
        )
...
def assign_moderators(
    ...
    if response.status_code == 422:
        warn(
            "One or more configured moderators "
            "could not be assigned. Confirm that "
            "they are repository collaborators."
        )
...
def intake_comment_exists() -> bool:
    ...
    while True:
        ...
        for comment in comments:
            ...
            if INTAKE_COMMENT_MARKER in body:
                return True

            if any(
                marker in body
                for marker in LEGACY_INTAKE_COMMENT_MARKERS
            ):
                return True
    ...
    lines.extend(
        [
            ...
            (
                ...
                "Official-source research and human "
                "review still follow."
            ),
        ]
    )
