---
schema_version: 2
record_type: opportunity
slug: euroavia-ideathon-2026
title: EUROAVIA Ideathon 2026
organizer: EUROAVIA (hosted by AS Oostende)
main_category: competitions
category: hackathon
edition: '2026'
status: pending-review
summary: EUROAVIA Ideathon 2026 is an in-person hackathon in Brugge, Belgium, where bachelor’s and master’s students tackle real-world aerospace industry challenges in teams. Participants gain industry exposure, networking opportunities, and hands-on innovation experience. The event is conducted in English and welcomes students aged 18 and above enrolled at a university.
format: in-person
location:
  display: In-person in Brugge, Belgium
  host_city: Brugge
  host_country: Belgium
  host_country_code: BE
  additional_locations: []
dates:
  application_deadline:
    raw: '27.07'
    display: 27 July 2026
    normalized: '2026-07-27'
  start_date:
    display: 3 November 2026
    normalized: '2026-11-03'
  end_date:
    display: 8 November 2026
    normalized: '2026-11-08'
  additional_dates: []
eligibility:
  geographic_regions: []
  eligible_countries: []
  nationality_or_residency_rules: Open to bachelor’s and master’s students enrolled at a university at the time of application; no nationality restriction mentioned.
  academic_levels:
  - undergraduate-students
  - master's-students
  broad_fields:
  - engineering
  specific_majors:
  - aerospace
  age_requirements: 18+
  experience_requirements: Must be enrolled as bachelor’s or master’s student at a university.
  language_requirements: English (event language)
  display_points:
  - Open to bachelor’s and master’s students enrolled at a university
  - Applicants must be at least 18 years old
  - Event conducted in English
  - No nationality restriction mentioned
  - 'Relevant fields: engineering, aerospace'
audience:
  classification_source: submitted-dropdown-only
  groups: []
  access_model: not-confirmed
  display_points: []
funding:
  application_fee: null
  participation_fee: null
  scholarship: null
  travel_support: null
  accommodation: null
  meals: null
  stipend_or_salary: null
  prizes: null
  visa_support: null
  accessibility_support: null
  other_support: []
  display_points: []
application:
  official_page: https://ideathon.euroavia.eu
  application_page: https://ideathon.euroavia.eu/applications
  requirements:
  - Team of 3–5 members (or individual applicants grouped into teams)
  - Bachelor’s or master’s student enrolled at a university
  - Age 18+
  documents: []
  selection_process:
  - Solutions evaluated by jury of industry and academic experts
  - Team applications prioritized
  display_points:
  - Apply online via the official application portal
  - Form a team of 3–5 members or apply individually to be grouped
  - Eligibility documents may be requested
program:
  activities:
  - Hackathon-style competition
  - Solving four real-world industrial challenges
  - Teamwork
  benefits:
  - Industry exposure
  - Networking
  - Innovation experience
  topics:
  - Aerospace industry challenges
  - Innovation
  - Engineering
filters:
  main_categories:
  - competitions
  categories:
  - hackathon
  formats:
  - in-person
  host_countries:
  - belgium
  eligible_regions: []
  eligible_countries: []
  academic_levels:
  - undergraduate-students
  - master's-students
  academic_fields:
  - engineering
  subjects:
  - aerospace
  audience_groups: []
  funding_features: []
  topics:
  - aerospace-industry-challenges
  - innovation
  - engineering
tags:
- hackathon
- aerospace
- engineering
- student-competition
- innovation
verification:
  human_review_required: true
  safe_to_generate_draft_page: true
  research_confidence: 90
  research_recommended_action: continue-to-human-review
  formatter_recommended_action: continue-to-draft-pr
  identity_categories_locked_to_research: true
  audience_groups_locked_to_research: true
  automatically_verified: false
  automatically_published: false
submission:
  issue_number: 19
  issue_url: https://github.com/elchrysaki/offmap-hub/issues/19
provenance:
  research_model: openai/gpt-4.1
  formatter_model: openai/gpt-4.1
  researched_at: '2026-07-18T17:52:12.946027+00:00'
  formatted_at: '2026-07-18T17:52:23.416294+00:00'
  generated_at: '2026-07-18T17:52:23.525970+00:00'
---

# ð§­ EUROAVIA Ideathon 2026

> EUROAVIA Ideathon 2026 is an in-person hackathon in Brugge, Belgium, where bachelor’s and master’s students tackle real-world aerospace industry challenges in teams. Participants gain industry exposure, networking opportunities, and hands-on innovation experience. The event is conducted in English and welcomes students aged 18 and above enrolled at a university.

## Opportunity Overview

EUROAVIA Ideathon 2026 invites university students to Brugge, Belgium for a hackathon focused on aerospace industry challenges. Teams collaborate to solve real-world problems, connect with industry professionals, and gain valuable innovation experience.

## Organizer

EUROAVIA is a European association for aerospace students, with AS Oostende hosting the 2026 Ideathon.

## Who Can Apply

Bachelor’s and master’s students enrolled at any university, aged 18 or older, are eligible. No nationality restriction is mentioned. The event is conducted in English and is relevant for those studying engineering or aerospace.

## Location & Format

The event takes place in person in Brugge, Belgium.

## Important Dates

Applications close on 27 July 2026. The Ideathon runs from 3 to 8 November 2026.

## Funding & Support

No information about fees, scholarships, or travel/accommodation support is confirmed. Applicants should check the official website for updates.

## How to Apply

Apply online through the official portal. Form a team of 3–5 members or apply individually to be grouped. Solutions are evaluated by a jury of industry and academic experts.

## What Participants Do

Participants work in teams to solve four real-world aerospace industry challenges, engage in innovation activities, and network with peers and professionals.

## ð Official Links

- [Official opportunity page](<https://ideathon.euroavia.eu>)
- [Direct application page](<https://ideathon.euroavia.eu/applications>)

---

### Review trail

This draft was prepared from a community submission and official-source research. Human approval is still required before publication.

- [View the original submission](<https://github.com/elchrysaki/offmap-hub/issues/19>)
